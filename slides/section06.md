...
<!-- _footer: "<div>pdo_sqlite3 の有効化（Windows）</div>" -->
...
if ($method === "GET" && $id === null) {
  ...
  $stmt = $pdo->prepare("SELECT * FROM contacts WHERE id = ?");
  $stmt->execute([$id]); $contact = $stmt->fetch(PDO::FETCH_ASSOC);
  echo $contact === false ? json_encode(["error" => "Not Found"]) : json_encode($contact, JSON_UNESCAPED_UNICODE); exit;
  ...
  http_response_code(201); echo json_encode(["id" => $pdo->lastInsertId(), "message" => "Created"]); exit;
  ...
  echo json_encode(["message" => "Updated"]); exit;
} else if ($method === "DELETE" && $id !== null) {
  $stmt = $pdo->prepare("DELETE FROM contacts WHERE id = ?");
  $stmt->execute([$id]); echo json_encode(["message" => "Deleted"]); exit;
}
